...
int inserir_ArvRN_Automatico(ArvRNPort **Raiz, InfArvRN Info, int unidade, InfoBB Info_ingles){
    int situacao; 
    situacao = 1; //significa que deu certo
    ...
    //situacao, se for 0, é falha na alocação, se for 1 o novo nó foi criado e preenchido, se for 2 o nó já existia e o utilizaremos pra fazer a inserção na lista
    ...
    if(situacao != 0){
        ...
        situacao = inserir_ArvBB_Automatico((&(no_referencia)->info.significados_ingles), Info_ingles, unidade);
    ...
    if (*Raiz == NULL) {
        ...
        for (int i = 0; i < contador_palavras; i++) {
            ...
            for (int j = 0; j < palavras_importadas[i].num_traducoes; j++) {
                ...
                situacao = inserir_ArvRN_Automatico(Raiz, info_port, unidade, info_ing);

                //printf("Situação na função de dicionario: %d\n", resultado); 
