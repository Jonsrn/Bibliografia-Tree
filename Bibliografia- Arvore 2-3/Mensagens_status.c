...
void mensagem_status_exclusao_correspondente_ingles(int situacao){
    ...
    if(situacao == 1){
         printf("A operação foi realizada com sucesso, palavra em português removida da árvore 2-3.\n");
    }
    if(situacao == 2){
    ...
    if(situacao == 4){
        printf("A operação falhou porque a remoção da palavra em português na 2-3 não foi bem sucedida.\n");
    }
    if(situacao == 5){
